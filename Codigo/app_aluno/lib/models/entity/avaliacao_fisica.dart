...
class AvaliacaoFisica {
  ...
  void _calcularIdade() {
    if (aluno != null) {
      idade = DateTime.now().difference(aluno!.dataNascimento).inDays ~/ 365;
    }
  }
  ...
  void _calcularRelacaoCinturaQuadril() {
    if (_medidaCintura != null && _medidaQuadril != null) {
      _relacaoCinturaQuadril = _medidaCintura! / _medidaQuadril!;
    }
  }

  void _calcularPercentualGordura() {
    if (aluno != null &&
        _medidaCintura != null &&
        _medidaPescoco != null &&
        _altura != null) {
      if (aluno!.sexo == Sexo.feminino.label && _medidaQuadril != null) {
        _calcularPercentualGorduraMulheres();
      } else if (aluno!.sexo == Sexo.masculino.label) {
        _calcularPercentualGorduraHomens();
      }
    }
  }

  void _calcularPesoGordura() {
    if (_peso != null && _percentualGordura != null) {
      _pesoGordura = _peso! * (_percentualGordura! / 100);
    }
  }
  ...
  void calcularIndices() {
    _calcularIdade();
    _calcularIMC();
    _calcularRelacaoCinturaQuadril();
    _calcularPercentualGordura();
    _calcularPesoGordura();
  }

  void _calcularPercentualGorduraHomens() {
    _percentualGordura =
        86.010 * (log(_medidaCintura! - _medidaPescoco!) / ln10) -
            70.041 * (log(_altura!) / ln10) +
            36.76;
  }

  void _calcularPercentualGorduraMulheres() {
    _percentualGordura = 163.205 *
            (log(_medidaCintura! + _medidaQuadril! - _medidaPescoco!) / ln10) -
        97.684 * (log(_altura!) / ln10) -
        78.387;
  }
