...
void main() {
  ...
  group('RealizacaoTreinoRepository', () {
    ...
    setUp(() async {
      ...
      final pacoteRef =
          await instance.collection('pacotes').add(pacote.toMap());
      pacote.id = pacoteRef.id;
      ...
      final alunoRef = await instance.collection('alunos').add(aluno.toMap());
      aluno.id = alunoRef.id;
    });
    ...
    test('readAllByAlunoByMes returns list of RealizacaoTreino', () async {
      ...
      expect(realizacoesTreino[2].data, DateTime(2023, 10, 10));
      expect(realizacoesTreino[2].treino.id, treino.id);
      expect(realizacoesTreino[2].tempo, 3600);
      ...
      expect(realizacoesTreino[0].data, DateTime(2023, 10, 12));
      expect(realizacoesTreino[0].treino.id, treino.id);
      expect(realizacoesTreino[0].tempo, 3400);
    ...
    test('findAllUltimaSemana returns list of RealizacaoTreino', () async {
      final hoje = DateTime.now();

      final realizacoesTreino =
          await realizacaoTreinoRepository.findAllUltimaSemana(hoje);
