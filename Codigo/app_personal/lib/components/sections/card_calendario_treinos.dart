...
class _CardCalendarioTreinosState extends State<CardCalendarioTreinos> {
          ...
          child: PageView.builder(
            ...
            itemBuilder: (context, index) {
              DateTime displayMonth =
                  DateTime(widget.hoje.year, widget.hoje.month - (2 - index));
              return _buildCalendar(displayMonth, colorTheme);
            },
          ),
        ),
      ],
  ...
  Widget _buildCalendar(DateTime month, ColorFamily colorTheme) {
    List<TableRow> rows = [];
          ...
          child: SizedBox(
            ...
            child: Container(
              decoration: BoxDecoration(
                color: isHoje ? colorTheme.lemon_secondary_500 : null,
                shape: BoxShape.circle,
                border: isNaoTreinado && !isHoje
                    ? Border.all(color: colorTheme.red_error_500)
                    : isTreinado && !isHoje
                        ? Border.all(color: colorTheme.indigo_primary_800)
                        : null,
              ),
              child: Center(
                child: Text(
