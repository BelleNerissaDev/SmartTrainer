...
import 'package:SmartTrainer_Personal/components/drawers/copiar_plano_aluno_bottonsheet.dart';
import 'package:SmartTrainer_Personal/components/loading/loading.dart';
import 'package:SmartTrainer_Personal/components/sections/plano_treino/alert_dialog_novo_plano_treino.dart';
...
class NovoPlanoTreino extends StatefulWidget {
  const NovoPlanoTreino({Key? key, this.alunoIdOnTest}) : super(key: key);
...
class _NovoPlanoTreinoState extends State<NovoPlanoTreino> {
  late Aluno aluno;
  ...
  List<PlanoTreino> _planoCopia = [];

  String? _searchQuery;
  bool showTreinos = false;
  bool isEditing = false;

  void _initializeTreinos() {
    _treinos = [
      Treino(nome: 'Treino A', exercicios: []),
    ];
  }
  ...
  Future<void> _fetchPlanoAlunoCopia(
      String alunoId, BuildContext context) async {
    try {
      showLoading(context);
      final PlanoTreinoController planoTreinoController =
          PlanoTreinoController();

      List<PlanoTreino> plano =
          await planoTreinoController.fetchPlanoTreinoAtivo(alunoId);

      if (plano.isNotEmpty) {
        setState(() {
          _planoCopia = plano;
          _treinos = plano[0].treinos;
        });
        _nomeController.text = _planoCopia[0].nome;
        showTreinos = true;
  ...
  Future<void> _criarPlanoTreino(ColorFamily colorTheme) async {
    ...
    try {
      await novoPlanoTreinoController.criarPlanoTreino(
        alunoId: aluno.id!,
        plano: plano,
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Novo plano de treino criado!'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Erro ao criar o plano de treino: $e'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                                  ...
                                  showTreinos
                                      ...
                                      : Column(
                                          children: [
                                            const Text(
                                                // ignore: lines_longer_than_80_chars
                                                'Escolha para iniciar um treino'),
                                            const SizedBox(height: 16),
                                            PrimaryButton(
                                              icon: Icons.add_circle,
                                              label: 'Novo plano de treino',
                                              onPressed: () {
                                                setState(() {
                                                  showTreinos = true;
                                                });
                                            ...
                                            PrimaryButton(
                                              ...
                                              label: 'Copiar de outro aluno',
                                              onPressed: () async {
                                                final alunoSelecionado =
                                                    // ignore: lines_longer_than_80_chars
                                                    await showStudentListBottomSheet(
                                                        context, colorTheme);
                                                if (alunoSelecionado != null) {
                                                  _fetchPlanoAlunoCopia(
                                                      alunoSelecionado.id!,
                                                      context);
                                                } else {
                                                  Exception(
      ...
      floatingActionButton: showTreinos
          ? FloatingActionButton(
              onPressed: () {
                if (_nomeController.text.isEmpty) {
                  showMessageSnackBar(
                      colorTheme, context, 'Insira o nome do plano',
                      error: true);
                } else if (_treinos
                    .any((treino) => (treino.exercicios).isEmpty)) {
                  showMessageSnackBar(colorTheme, context,
                      'Todos os treinos devem ter pelo menos um exercício',
                      error: true);
                } else {
                  AlertNovoPlanoTreino.show(
                      context, colorTheme, _treinos, _criarPlanoTreino);
                }
              },
